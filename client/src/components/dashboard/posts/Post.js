...
const Post = ({ post }) => {
    ...
    const likePost = async () => {
        try {
            const response = await fetch("http://localhost:5000/dashboard/post/" + post.post_id + "/like", {
                method: "POST",
                headers: { token: localStorage.token }
            });
        } catch (err) {
    ...
    const retweetPost = async () => {
        try {
            const response = await fetch("http://localhost:5000/dashboard/post/" + post.post_id + "/retweet", {
                method: "POST",
                headers: { token: localStorage.token }
            });
        } catch (err) {
        ...
        <div>
            <div className='post'>
                {post.retweet_user_id && <div className="activity-info">
                    <span>{post.retweet_user_id} Retweeted</span>
                </div>}
